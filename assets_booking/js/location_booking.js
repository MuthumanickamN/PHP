jQuery(document).ready(function(){
    ...
    function get_location_details(){
        ...
        $.ajax({ 	
            type: "POST",   
            url: base_url+"location_booking/get_location_details",
            //data:"date="+date,		
            async: false,
            datatype: "json",
    ...
    if(stid==0)
	{
		...
		$(this).children("i").addClass("fa fa-times");
		  ...
		  $(this).attr("stat_at", statid);
			...
			$.ajax({ 	
            type: "POST",   
            url: base_url+"location_booking/location_status_update",
	...
	jQuery(document).on('click','.edit_location', function(e){
        ...
        $.ajax({ 	
            type: "POST",   
            url: base_url+"location_booking/get_location",
            data:"id="+id,		
            async: false,
            datatype: "json",
            success : function(data)
		...
		if (confirm("Are you sure you want to delete this record?")) {
			...
			$.ajax({ 	
				type: "POST",   
				url: base_url+"location_booking/delete_location",
				data:"id="+id,		
				async: false,
				datatype: "html",
				success : function(data)
