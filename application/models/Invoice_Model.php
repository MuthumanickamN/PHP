...
class Invoice_Model extends CI_Model {
	...
	public function send_email_invoice($wallet_transaction_id, $from=''){
		...
		else if(trim($from) == "CourtBooking")
	    {
	        $slot_booking_id = $row['slot_booking'];
	        if(!$slot_booking_id)
	        {
		...
		else if(trim($from) == "CourtBookingApproval")
	    {
	        $slot_booking_id = $row['slot_booking'];
	        if(!$slot_booking_id)
	        {
	        ...
	        left join sports s on bs.sid=s.id
	        left join parent p on p.parent_id = b.customerid
	        where b.id=$slot_booking_id and b.bstatus=1 and b.blocked_status=0 group by b.id";
	        $result2 = $this->db->query($sql2)->row_array();
		...
		else if($from == "Contract")
	    {
            ...
            $body .= "<tr>
                        <td>1.</td>
                        <td> Contract Amount of ".date('M Y')."</td>
                        <td>".$row['gross_amount']."</td>
                        <td>".$row['vat_value']."</td>
                        </tr>
                        <tr>
