...
class Registration_fees extends CI_Controller {  
	...
	public function index(){
		...
		$query = $this->db->query("select * from locations ".$where);
		$locationListing = $query->result_array();
		foreach($locationListing as $key=>$location)
		{
			$locationListing[$key]['location_id']=$this->transaction->getLocationDetail($location['location_id']);
		}
		...
		if($this->input->post('submit')){
			...
			if( $creditsDetails['balance_credits'] >= $payable_amount){
			    $sql="INSERT into registration_fees(student_id,student_name,location_id,parent_id,parent_name,parent_contact,reg_fee_category,reg_fee,pay_type,wallet_balance,wallet,created_at,status,vat_percent, vat_value,pay_date, net_amount,fee_pay_type ) values('".$student_id."','".$student_name."','".$location_id."','".$parent_id."','".$parent_name."','".$parent_contact_no."','".$category."','".$reg_fee_amount."','".$payment_type."','".$balance_credits."','".$balance_credits."','".$created_at."','Active','".$vat_percent."','".$vat_value."','".date('Y-m-d')."','".$payable_amount."','Registration Fees')";
...
public function edit($id)
{
    ...
    $data['wallet_balance']=$postData['wallet_balance'];
		...
		$updated_at=currentDateTime();
		 ...
		 $sql="Update  registration_fees set student_id='$student_id',location_id='$location_id',parent_id='$parent_id',parent_name='$parent_name',parent_contact='$parent_contact_no',reg_fee_category='$category',pay_type='$payment_type',reg_fee='$reg_fee_amount',wallet_balance='$payable_amount',wallet='$wallet_amount',updated_at='$updated_at' where id='$id'";
		$insert=$this->db->query($sql);
...
public function list_(){
	...
	foreach ($data['data'] as $key => $value) 
	{
        ...
        $data['data'][$key]['location_id'] = $this->transaction->getLocationDetail($value['location_id']);
